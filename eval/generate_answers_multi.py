...
graphs = [
    "pruned_adaptive_multi_strategy",
    "pruned_adaptive_multi_strategy_tuned",
    "pruned_crumbtrail",
    "pruned_crumbtrail_tuned",
    "pruned_kgtrimmer",
    "pruned_kgtrimmer_tuned",
    "pruned_pathrag_hybrid",
    "pruned_pathrag_hybrid_tuned",
    "pruned_pog_hybrid",
    "pruned_pog_hybrid_tuned",
]
...
# --------------------------------------------------------------------------------
def load_graphrag_artifacts(workspace: Path, graph: str):
    """Load all necessary GraphRAG artifacts for API querying."""
    unpruned_dir = workspace / "output"
    ...
    community_reports = pd.read_parquet(unpruned_dir / "community_reports.parquet")
    text_units = pd.read_parquet(unpruned_dir / "text_units.parquet")

    # Load documents for ID mapping
    documents = pd.read_parquet(unpruned_dir / "documents.parquet")
...
async def generate_answers_api(
    ...
    for graph in graphs:
        ...
        output_file = output_dir / f"{graph}.json"
        # Load GraphRAG artifacts once
        artifacts = load_graphrag_artifacts(workspace_path, graph)
...
def main():
    ...
    out_dir = (
        Path("../data/generated")
        / args.search_method
        / f"generated_answers_{args.search_method}_{timestamp}"
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    asyncio.run(
        generate_answers_api(
            questions, args.workspace, args.search_method, out_dir, args.parallel
        )
    )
    return 0
